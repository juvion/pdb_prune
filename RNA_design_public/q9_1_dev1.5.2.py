...
from sequence_similarity_score2 import RNASequenceScorer, load_reference_matrix
...
# 对文件列表进行排序以确保可复现性
train_file_list = sorted(os.listdir("./RNAdesignv1/train/seqs"))
content_dict = {
    "pdb_id": [],
    "seq": []
}
...
class TrainConfig:
    batch_size: int = 16
    epoch: int = 200  # 总共200个epoch：前150个训练QCBM，后50个固定QCBM训练RNA模型
    qcbm_epochs: int = 150  # 前150个epoch训练QCBM
    rna_epochs: int = 50   # 后50个epoch固定QCBM训练RNA模型
    lr: float = 0.001
    output_dir: str = 'ckpts/q9_1_dev1.4'
    ckpt_path: str = 'ckpts/q9_1_dev1.4/best_q9_1_dev1.4.pt'
    best_qcbm_path: str = 'ckpts/q9_1_dev1.4/best_qcbm.pt'  # 保存最佳QCBM参数
...
def featurize(batch):
    ...
    # Coordinate lengths from backbone atom P (all atoms share the same first dimension)
    coord_lengths = np.array([b['coords']["P"].shape[0] for b in batch], dtype=np.int32)
    # Effective lengths per item: ensure alignment by using the minimum of seq and coords
    eff_lengths = np.minimum(seq_lengths, coord_lengths)

    # Pad to the maximum effective length across the batch
    ...
    for i, b in enumerate(batch):
        ...
        seq_len = len(b['seq'])
        coords_len = x.shape[0]
        eff_len = int(min(seq_len, coords_len))
        if seq_len != coords_len:
            pass

        # Trim both coordinates and sequence indices to the effective length
        x_eff = x[:eff_len, :, :]
        indices = np.asarray([alphabet.index(a) for a in b['seq'][:eff_len]], dtype=np.int32)

        # Pad to L_max
        x_pad = np.pad(x_eff, [[0, L_max - eff_len], [0,0], [0,0]], 'constant', constant_values=(np.nan, ))
        X[i,:,:,:] = x_pad
        S[i, :eff_len] = indices
        names.append(b['name'])

    # Compact valid rows (remove all-NaN padded rows)
    ...
    # Return original sequence lengths for downstream compatibility
    return X, S, mask, seq_lengths, names
...
class SimpleQCBM:
    """量子电路玻尔兹曼机，基于PennyLane框架实现（qcbm2结构：每层 Rx-RZ -> 环形配对 RXX/IsingXX(参数化) -> RZ-RX）"""
    def __init__(self, num_qubits, num_layers, total_sequences, device='cpu'):
        ...
        # 参数：每层每比特4个旋转参数（Rx、RZ、RZ、Rx）+ 每层 num_qubits 个 IsingXX 纠缠角度参数（环形配对，参数化）
        self.pair_count = self.num_qubits
        self.num_params = self.num_layers * self.num_qubits * 4 + self.num_layers * self.pair_count
        self.weights = pnp.random.uniform(0.0, 2 * pnp.pi, size=(self.num_params,))

        # 创建量子电路用于采样（自定义结构）
        self.qcircuit_sample = qml.QNode(self._circuit_sample, self.qml_device, interface='numpy')
        # 创建用于概率计算的PennyLane设备（解析模式，无shots）
        self.qml_device_probs = qml.device('default.qubit', wires=self.num_qubits, shots=None)
        # 创建量子电路用于概率计算（返回所有基态的概率向量），显式使用 autograd 接口与 parameter-shift 以提供解析梯度
        self.qcircuit_probs = qml.QNode(self._circuit_probs, self.qml_device_probs, interface='autograd')

    def _circuit_sample(self, params):
        """定义参数化量子电路用于采样（Rx-RZ -> 环形配对 IsingXX -> RZ-Rx）"""
        ...
        entangler_weights = params[rot_size:].reshape(self.num_layers, self.pair_count)
        ...
        for l in range(self.num_layers):
            ...
            # 2) 全连接纠缠：每对量子比特使用独立参数的 IsingXX 门
            # 环形配对纠缠：每层使用 num_qubits 个参数，配对 (i, i+1 mod N)
            for i in range(self.num_qubits):
                j = (i + 1) % self.num_qubits
                qml.IsingXX(entangler_weights[l, i], wires=[i, j])
    ...
    def _circuit_probs(self, params):
        """定义参数化量子电路用于概率计算（Rx-RZ -> 环形配对 IsingXX -> RZ-Rx）"""
        wires = list(range(self.num_qubits))
        rot_size = self.num_layers * self.num_qubits * 4
        rotation_weights = params[:rot_size].reshape(self.num_layers, self.num_qubits, 4)
        entangler_weights = params[rot_size:].reshape(self.num_layers, self.pair_count)
    ...
    def get_sample_counts(self, num_samples=None):
        ...
        # 根据请求的采样数量选择合适的QNode/设备
        if num_samples != self.total_sequences:
            temp_device = qml.device('default.qubit', wires=self.num_qubits, shots=int(num_samples))
            temp_qnode = qml.QNode(self._circuit_sample, temp_device, interface='numpy')
            raw_samples = temp_qnode(self.weights)
        else:
            raw_samples = self.qcircuit_sample(self.weights)
    ...
    def set_parameters(self, params_vector):
        """从扁平化向量设置参数（直接使用一维参数向量）"""
        self.weights = pnp.array(params_vector)
    ...
    def load_state_dict(self, state_dict):
        """从状态字典加载模型参数，兼容旧字段并自动匹配新电路参数长度"""
        # 先更新结构参数以便正确计算参数总数
        self.num_qubits = state_dict.get('num_qubits', self.num_qubits)
        self.num_layers = state_dict.get('num_layers', self.num_layers)
        self.total_sequences = state_dict.get('total_sequences', self.total_sequences)
        self.pair_count = self.num_qubits
        self.num_params = self.num_layers * self.num_qubits * 4 + self.num_layers * self.pair_count

        # 加载权重并适配到一维向量
        if 'weights' in state_dict:
            loaded = state_dict['weights']
        elif 'rotation_params' in state_dict:
            loaded = state_dict['rotation_params']
        else:
            loaded = pnp.random.uniform(0.0, 2 * pnp.pi, size=(self.num_params,))

        loaded_vec = pnp.array(loaded).flatten()
        if loaded_vec.size != self.num_params:
            loaded_vec = pnp.random.uniform(0.0, 2 * pnp.pi, size=(self.num_params,))
        self.weights = loaded_vec
...
class RNAModelWithQCBM(nn.Module):
    ...
    def compute_package_scores(self, X, S, mask, lengths, prior_samples, sequence_indices, pdb_ids, epoch=None):
        ...
        with torch.no_grad():
            ...
            # 碱基映射：0->A, 1->U, 2->C, 3->G
            base_map = {0: 'A', 1: 'U', 2: 'C', 3: 'G'}

            # 按序列计算package_score分数
            package_scores = []
            start_idx = 0
            zero_bp_score_count = 0  # 统计base_pair_score为0的样本数量
            ...
            for i, length in enumerate(lengths):
                ...
                try:
                    ...
                    # 加载参考碱基对矩阵（优先使用基于PDB ID的文件，否则回退为单位矩阵）
                    matrices_dir = "./RNAdesignv1/train/matrices"
                    pdb_id = pdb_ids[i] if pdb_ids is not None and i < len(pdb_ids) else None
                    seq_len = len(true_sequence)
                    ref_matrix = None
                    if pdb_id is not None:
                        candidates = [
                            os.path.join(matrices_dir, f"{pdb_id}.txt"),
                            os.path.join(matrices_dir, f"{pdb_id}.csv"),
                            os.path.join(matrices_dir, f"{pdb_id.upper()}.txt"),
                            os.path.join(matrices_dir, f"{pdb_id.upper()}.csv"),
                            os.path.join(matrices_dir, f"{pdb_id.lower()}.txt"),
                            os.path.join(matrices_dir, f"{pdb_id.lower()}.csv"),
                        ]
                        ref_matrix_path = None
                        for cand in candidates:
                            if os.path.exists(cand):
                                ref_matrix_path = cand
                                break
                        if ref_matrix_path is None:
                            pass
                        else:
                            try:
                                ref_matrix = load_reference_matrix(ref_matrix_path)
                                if ref_matrix.shape[0] != seq_len or ref_matrix.shape[1] != seq_len:
                    ...
                    # 统计base_pair_score是否为0
                    base_pair_score = combined_score_result['base_pair_score']
                    edit_distance_score = combined_score_result['edit_distance_score']
                    total_samples += 1
                    print(f"Sample {i}: base_pair_score = {base_pair_score}")
                    if base_pair_score == 0:
                        zero_bp_score_count += 1
                        print(f"  -> base_pair_score is ZERO for sample {i}")
                    else:
                        print(f"  -> base_pair_score is NON-ZERO for sample {i}: {base_pair_score}")
                        # 只在第一个epoch时删除已存在的日志文件
                        if epoch == 1 and os.path.exists('base_pair_q9_1_dev1.5.log'):
                            os.remove('base_pair_q9_1_dev1.5.log')
                        with open('base_pair_q9_1_dev1.5.log', 'a') as f:
                             f.write(f"NON-ZERO base_pair_score: {base_pair_score}\n")
                             f.write(f"edit_distance_score: {edit_distance_score}\n")
            ...
            # 输出统计结果
            print(f"\n{'='*50}")
            print(f"BASE_PAIR_SCORE STATISTICS:")
            print(f"Total samples: {total_samples}")
            print(f"Samples with base_pair_score = 0: {zero_bp_score_count}")
            print(f"Samples with base_pair_score > 0: {total_samples - zero_bp_score_count}")
            if total_samples > 0:
                zero_percentage = (zero_bp_score_count / total_samples) * 100
                print(f"Percentage of samples with base_pair_score = 0: {zero_percentage:.2f}%")
            print(f"{'='*50}\n")
...
class QCBMTrainer:
    ...
    def train_qcbm_epoch(self, train_loader, optimizer, epoch=None):
        """训练一个epoch"""
        ...
        # 3. 构建目标概率分布并优化QCBM参数
        self._optimize_qcbm_parameters(prior_scores, epoch)
    ...
    def _compute_prior_scores(self, train_loader):
        """构造完整目标分布：评分子集 + 汉明核扩散 + ε 背景归一化。保持后续优化流程不变。"""
        print("Computing prior sample scores with Hamming-kernel diffusion and epsilon background...")

        # 超参数（可按需调整或提到配置中）
        beta = 1.0   # 评分软化温度：softmax(scores / beta)
        tau = 1.0    # 汉明核温度：K(d) = exp(-d / tau)
        epsilon = 1e-6  # 背景常数，确保全域非零支撑

        # 1) 构造按字典序排列的所有 2**n 先验样本（n 为量子比特数），作为全集 U
        num_qubits = self.qcbm_model.num_qubits
        num_prior = 2 ** num_qubits

        # 从 QCBM 多次采样，统计频率并按频率降序构造评分子集
        sample_multiplier = 10  # 采样倍数，可调节以提升频率统计稳定性
        counts = self.qcbm_model.get_sample_counts(num_samples=int(self.qcbm_model.total_sequences * sample_multiplier))
        sample_keys = list(counts.keys())
        sample_freqs = np.array([counts[k] for k in sample_keys], dtype=float)
        order = np.argsort(-sample_freqs)
        sampled_bitstrings = [sample_keys[i] for i in order]

        # 采样得到的唯一本串集合，作为评分子集 S
        prior_samples_list = [[int(c) for c in bs] for bs in sampled_bitstrings]

        # 同时构造全集 U（按二进制顺序），用于汉明核扩散与 qml.probs 的索引一致性
        full_U_list = []
        unique_sample_keys = []
        for idx in range(num_prior):
            bits = [(idx >> (num_qubits - 1 - b)) & 1 for b in range(num_qubits)]
            full_U_list.append(bits)
            unique_sample_keys.append(tuple(bits))

        # 张量形式（置于正确的设备）
        prior_samples_tensor = torch.tensor(prior_samples_list, dtype=torch.float32, device=self.device)
        U_tensor_full = torch.tensor(full_U_list, dtype=torch.float32, device=self.device)

        # 2) 在训练集中按批计算与评分子集 S 的配对分数
        all_scores = []              # 与 S 中元素一一对应的分数
        scored_indices = []          # 记录 S 中元素在采样子集中的索引
        total_sequences_processed = 0
        M_unique = len(prior_samples_list)

        with torch.no_grad():
            for batch in tqdm(train_loader, desc="Computing scores"):
                if total_sequences_processed >= M_unique:
                    break
                ...
                # 可用序列数量与剩余需求（基于采样子集大小）
                batch_seq_count = len(lengths)
                remain = M_unique - total_sequences_processed
                use_seqs = min(batch_seq_count, max(remain, 0))

                # 构造 sequence_indices：为本批每个碱基分配先验样本索引（循环映射到采样子集）
                sequence_indices = []
                for j, length in enumerate(lengths):
                    if j < use_seqs and M_unique > 0:
                        prior_idx = (total_sequences_processed + j) % M_unique
                        scored_indices.append(prior_idx)
                    else:
                        prior_idx = 0  # 占位索引，仅用于形状一致，后续不计入评分子集
                    sequence_indices.extend([prior_idx] * length.item())

                sequence_indices = torch.tensor(sequence_indices, device=self.device)
                batch_pdb_ids = names

                # 计算 package_score 分数（基于采样得到的先验子集）
                package_scores = self.rna_model_with_qcbm.compute_package_scores(
                    X, S, mask, lengths, prior_samples_tensor, sequence_indices, batch_pdb_ids
                )

                # 仅保留前 use_seqs 个序列对应的分数
                all_scores.extend(package_scores[:use_seqs])
                total_sequences_processed += use_seqs

        # 若评分子集为空，退化为均匀分布
        if len(all_scores) == 0 or len(scored_indices) == 0:
            print("No scored prior samples found; falling back to uniform distribution.")
            target_probs = torch.full((num_prior,), 1.0 / num_prior, device=self.device)
            self.unique_sample_keys = unique_sample_keys
            return target_probs

        # 3) 将评分子集的分数转换为权重 p_S，并进行汉明核扩散到全集 U
        unique_scores = torch.tensor([float(score) for score in all_scores], device=self.device)
        with open('all_scores_dev1.5.log', 'a') as f:
            list_str = '[' + ', '.join([str(float(score)) for score in all_scores]) + ']'
            f.write('all_scores ' + list_str + '\n')

        # 评分软化（温度 beta）：得到子集权重 p_S，形状 [|S|]
        p_S = F.softmax(unique_scores / beta, dim=0)

        # 取出评分子集的比特串张量 S_tensor，形状 [|S|, num_qubits]
        S_tensor = prior_samples_tensor[torch.tensor(scored_indices, device=self.device)]  # [|S|, n]
        # 使用全集 U_tensor_full 进行扩散
        # 计算 U×S 的汉明距离矩阵，形状 [|U|, |S|]
        # 方案：逐位比较不等，计数为距离
        dist_bool = (U_tensor_full.unsqueeze(1) != S_tensor.unsqueeze(0))  # [|U|, |S|, n]
        dist = dist_bool.sum(dim=-1).float()                               # [|U|, |S|]

        # 汉明核权重：K(d) = exp(-d / tau)
        kernel = torch.exp(-dist / tau)                               # [|U|, |S|]

        # 扩散到全集：T_un = kernel · p_S（右乘），形状 [|U|]
        T_un = torch.matmul(kernel, p_S)

        # 加入 ε 背景并归一化
        T = T_un + epsilon
        target_probs = T / T.sum()

        # 日志记录
        with open('all_scores_dev1.5.log', 'a') as f:
            tp_iter = target_probs.detach().cpu().tolist()
            tp_list_str = '[' + ', '.join([str(float(p)) for p in tp_iter]) + ']'
            f.write('target_probs ' + tp_list_str + '\n')

        print(f"Number of prior samples (2**n): {num_prior}")
        print(f"Scored subset size: {len(unique_scores)}")
        print(f"Target probabilities shape: {target_probs.shape}")

        # 存储唯一样本键（与qml.probs的二进制顺序一致）供后续QCBM概率提取
        self.unique_sample_keys = unique_sample_keys

        return target_probs

    def _optimize_qcbm_parameters(self, prior_scores, epoch=None):
        ...
        def objective_function(params_vector):
            # 设置QCBM参数（将输入转换为 pnp.array 以启用 autograd 跟踪）
            params_vector = pnp.array(params_vector)
            self.qcbm_model.set_parameters(params_vector)

            # 使用解析概率计算所有基态的概率向量（直接对传入参数求值）
            full_probs = self.qcbm_model.qcircuit_probs(params_vector)
            ...
            # 计算KL散度 KL(target || pred): 直接使用full_probs与target_np
            eps = 1e-12
            full_probs_safe = pnp.clip(full_probs, eps, None)
            kl = pnp.sum(target_np * pnp.log(target_np / full_probs_safe))
        ...
        def optimization_callback(xk):
            ...
            # 计算当前KL散度（使用解析概率full_probs，保持与交叉熵一致的处理）
            try:
                full_probs = self.qcbm_model.qcircuit_probs(self.qcbm_model.weights)
                if isinstance(self.target_distribution, torch.Tensor):
                    target_np = self.target_distribution.cpu().numpy()
                else:
                    target_np = self.target_distribution
                eps = 1e-12
                full_probs_safe = pnp.clip(full_probs, eps, None)
                current_kl = pnp.sum(target_np * pnp.log(target_np / full_probs_safe))
                print(f"Iteration {iteration_count[0]:3d}: Loss = {current_loss:.6f}, KL = {current_kl:.6f}")
            except Exception as e:
                print(f"Iteration {iteration_count[0]:3d}: Loss = {current_loss:.6f}, KL计算错误: {e}")
        ...
        # 定义解析梯度（Jacobian）供 SciPy 使用
        def jac_wrapper(params_vector):
            grad_fn = qml.grad(objective_function)
            grad = grad_fn(pnp.array(params_vector))
            return np.array(grad, dtype=float)

        # 使用带解析梯度的 L-BFGS-B 方法进行优化
        result = minimize(
            objective_function,
            initial_params,
            method='SLSQP',
        ...
        # 计算最终的概率分布和KL散度（使用解析概率，直接用full_probs）
        full_probs = self.qcbm_model.qcircuit_probs(self.qcbm_model.weights)
        ...
        # 直接用full_probs计算KL散度，保持与交叉熵一致的处理
        eps = 1e-12
        full_probs_safe = pnp.clip(full_probs, eps, None)
        kl_divergence = pnp.sum(target_np * pnp.log(target_np / full_probs_safe))
        
        # 只在第一个epoch时删除已存在的KL散度日志文件
        if epoch == 1 and os.path.exists('KL_divergency_dev1.5.log'):
            os.remove('KL_divergency_dev1.5.log')
        # 保存KL散度到日志文件
        with open('KL_divergency_dev1.5.log', 'a') as f:
            if epoch is not None:
                f.write(f"Epoch {epoch}: {kl_divergence:.8f}\n")
            else:
                f.write(f"{kl_divergence:.8f}\n")
        
        # 只在第一个epoch时删除已存在的QCBM优化结果日志文件
        if epoch == 1 and os.path.exists('qcbm_optimization_results_dev1.5.log'):
            os.remove('qcbm_optimization_results_dev1.5.log')
        # 保存目标概率分布和最终QCBM概率分布到日志文件
        with open('qcbm_optimization_results_dev1.5.log', 'a') as f:
            # 保存目标概率分布（以列表格式）
            target_list_str = '[' + ', '.join([f"{float(prob):.8f}" for prob in target_np]) + ']'
            if epoch is not None:
                f.write(f"Epoch {epoch} Target: {target_list_str}\n")
            else:
                f.write(f"Target: {target_list_str}\n")
            # 保存QCBM概率分布
            pred_str = ' '.join([f"{float(prob):.8f}" for prob in full_probs])
            if epoch is not None:
                f.write(f"Epoch {epoch} QCBM: {pred_str}\n")
            else:
                f.write(f"QCBM: {pred_str}\n")
    ...
    def train_qcbm(self, train_loader, optimizer, num_epochs=30):
        ...
        for epoch in range(num_epochs):
            ...
            avg_loss = self.train_qcbm_epoch(train_loader, optimizer, epoch + 1)
...
def run_single_experiment(split_seed, model_seed=2025):
    ...
    best_qcbm_recovery = 0
    best_qcbm_params = None

    # 初始化记录列表
    training_losses = []
    validation_recoveries = []

    # 第一阶段：训练QCBM参数（前150个epoch）
    print(f"\n{'='*80}")
    print("第一阶段：训练QCBM量子参数（前150个epoch）")
    print(f"{'='*80}")
    
    for epoch in range(config.train_config.qcbm_epochs):
        print(f"\n{'='*60}")
        print(f"QCBM训练 Epoch {epoch + 1}/{config.train_config.qcbm_epochs}")
        print(f"{'='*60}")

        # 训练QCBM一个epoch
        avg_loss = qcbm_trainer.train_qcbm_epoch(train_loader, optimizer,epoch=epoch)
        print(f'Epoch {epoch + 1}/{config.train_config.qcbm_epochs}, Training Loss: {avg_loss:.4f}')
        ...
        with torch.no_grad():
            ...
            print(f'Epoch {epoch + 1}/{config.train_config.qcbm_epochs}, Validation Recovery: {valid_recovery:.4f}')

            # 记录验证恢复率
            validation_recoveries.append(valid_recovery)

            # 保存最佳QCBM参数
            if valid_recovery > best_qcbm_recovery:
                best_qcbm_recovery = valid_recovery
                best_qcbm_params = qcbm_model.state_dict().copy()
                torch.save({
                    'qcbm_model': qcbm_model.state_dict(),
                    'epoch': epoch,
                    'valid_recovery': valid_recovery
                }, config.train_config.best_qcbm_path)
                print(f"新的最佳QCBM参数已保存，验证恢复率: {valid_recovery:.4f}")

    # 第二阶段：固定最佳QCBM参数，继续训练RNA模型（后50个epoch）
    print(f"\n{'='*80}")
    print("第二阶段：固定最佳QCBM参数，继续训练RNA模型（后50个epoch）")
    print(f"{'='*80}")
    
    # 加载最佳QCBM参数
    if best_qcbm_params is not None:
        qcbm_model.load_state_dict(best_qcbm_params)
        print(f"已加载最佳QCBM参数（验证恢复率: {best_qcbm_recovery:.4f}）")
    
    # 注意：QCBM使用numpy数组，不需要冻结参数，只需要在训练时不更新即可
    # 第二阶段只训练RNA模型参数，QCBM参数保持固定
    
    # 重新创建优化器，只优化RNA模型参数（排除prior_embedding，因为它与QCBM相关）
    rna_params = []
    for name, param in rna_model_with_qcbm.named_parameters():
        if 'prior_embedding' not in name:  # 排除先验样本嵌入层，因为它与QCBM输出相关
            rna_params.append(param)
    rna_optimizer = Adam(rna_params, config.train_config.lr)
    
    for epoch in range(config.train_config.rna_epochs):
        print(f"\n{'='*60}")
        print(f"RNA模型训练 Epoch {epoch + 1}/{config.train_config.rna_epochs}")
        print(f"{'='*60}")

        # 训练RNA模型（QCBM参数固定）
        rna_model_with_qcbm.train()
        epoch_loss = 0
        num_batches = 0
        
        for batch in tqdm(train_loader, desc="Training RNA Model"):
            X, S, mask, lengths, names = batch
            X = X.to(config.device)
            S = S.to(config.device)
            mask = mask.to(config.device)
            ...
            prior_samples = qcbm_trainer.generate_prior_samples(train_dataset_size)
            
            # 为当前batch分配先验样本索引
            sequence_indices = []
            sequence_index = 0
            for length in lengths:
                seq_idx = sequence_index % train_dataset_size
                seq_indices = [seq_idx] * length.item()
                sequence_indices.extend(seq_indices)
                sequence_index += 1
            ...
            # 前向传播
            log_probs, S_feat, batch_id = rna_model_with_qcbm(X, S, mask, prior_samples, sequence_indices)
            
            # 计算损失 (log_probs已经是log_softmax的结果，需要转换为logits或使用nll_loss)
            criterion = nn.CrossEntropyLoss()
            loss = criterion(log_probs, S_feat)
        ...
        print(f'RNA模型训练 Epoch {epoch + 1}/{config.train_config.rna_epochs}, Training Loss: {avg_loss:.4f}')
        training_losses.append(avg_loss)

        # 验证
        rna_model_with_qcbm.eval()

        # 使用固定的QCBM重新采样验证集大小的先验样本
        valid_dataset_size = len(valid_dataset)
        validation_prior_samples = qcbm_trainer.generate_prior_samples(valid_dataset_size)
        ...
        with torch.no_grad():
            ...
            valid_recovery = np.mean(recovery_list)
            print(f'RNA模型训练 Epoch {epoch + 1}/{config.train_config.rna_epochs}, Validation Recovery: {valid_recovery:.4f}')

            # 记录验证恢复率
            validation_recoveries.append(valid_recovery)

            # 保存最佳RNA模型（使用最佳QCBM参数）
            if valid_recovery > best_valid_recovery:
                best_valid_recovery = valid_recovery
                torch.save({
                    'rna_model': rna_model_with_qcbm.state_dict(),
                    'qcbm_model': qcbm_model.state_dict(),
                    'epoch': config.train_config.qcbm_epochs + epoch,
                    'valid_recovery': valid_recovery,
                    'best_qcbm_recovery': best_qcbm_recovery
                }, config.train_config.ckpt_path)
                print(f"新的最佳RNA模型已保存，验证恢复率: {valid_recovery:.4f}")

    # 测试阶段：使用最佳QCBM参数和最佳RNA模型参数
    print("\n" + "="*80)
    print("测试阶段：使用最佳量子电路参数和RNA模型参数")
    print("="*80)

    # 加载最佳RNA模型（包含最佳QCBM参数）
    ...
    print(f"已加载最佳模型：")
    print(f"  - RNA模型来自第 {checkpoint['epoch']} 个epoch，验证恢复率: {checkpoint['valid_recovery']:.4f}")
    if 'best_qcbm_recovery' in checkpoint:
        print(f"  - QCBM参数来自前150个epoch的最佳结果，验证恢复率: {checkpoint['best_qcbm_recovery']:.4f}")

    rna_model_with_qcbm.eval()

    # 使用最佳QCBM参数重新采样测试集大小的先验样本
    ...
    with torch.no_grad():
        ...
        print(f'最终测试恢复率: {test_recovery:.4f}')
...
if __name__ == "__main__":
    ...
    for i, split_seed in enumerate(split_seeds):
        ...
        results_df.to_csv('qcbm_rna_results_q9_1_dev1.4_qbits9.csv', index=False)
        print(f"Results updated and saved to qcbm_rna_results_q9_1_dev1.4_qbits9.csv")
    ...
    results_df.to_csv('qcbm_rna_results_q9_1_dev1.4_qbits9.csv', index=False)
    print(f"\nResults saved to qcbm_rna_results_q9_1_dev1.4_qbits9.csv")
