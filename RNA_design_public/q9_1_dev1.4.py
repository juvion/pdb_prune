...
from sequence_similarity_score import RNASequenceScorerOptimized, load_reference_matrix
            ...
            for i, length in enumerate(lengths):
                ...
                try:
                    ...
                    combined_score_result = scorer.calculate_combined_score(
                        pred_seq_loaded, ref_seq_loaded, ref_matrix, lambda_param=0.3
                    )
                    ...
                    if base_pair_score == 0:
                        ...
                        if epoch == 1 and os.path.exists('base_pair_q9_1_dev1.4.log'):
                            os.remove('base_pair_q9_1_dev1.4.log')
                        with open('base_pair_q9_1_dev1.4.log', 'a') as f:
...
class QCBMTrainer:
    ...
    def _compute_prior_scores(self, train_loader):
        """计算先验样本的分数（使用2**n个按字典序排列的样本与前2**n个训练序列配对）"""
        print("Computing prior sample scores with full 2**n lexicographic bitstrings...")

        # 1) 构造按字典序排列的所有2**n先验样本（n为量子比特数）
        ...
        for idx in range(num_prior):
            # 生成长度为num_qubits的比特串，按字典序（等价于十进制递增）
            bits = [ (idx >> (num_qubits - 1 - b)) & 1 for b in range(num_qubits) ]
            prior_samples_list.append(bits)
        ...
        # 2) 从训练集中选择前2**n个序列，按train_loader的batch size分批计算分数
        all_scores = []
        total_sequences_processed = 0

        with torch.no_grad():
            for batch in tqdm(train_loader, desc="Computing scores"):
                if total_sequences_processed >= num_prior:
                    break
                ...
                remain = num_prior - total_sequences_processed
                use_seqs = min(batch_seq_count, remain)

                # 构造sequence_indices：为本批次每个碱基分配先验样本索引
                sequence_indices = []
                for j, length in enumerate(lengths):
                    # 前use_seqs个序列按照全局次序分配不同的先验样本，其余序列分配一个占位索引（如0），并在结果中忽略
                    if j < use_seqs:
                        prior_idx = total_sequences_processed + j
                    else:
                        prior_idx = 0  # 占位索引，仅用于保持张量形状一致
        ...
        target_probs = unique_scores / unique_scores.sum()
    ...
    def _optimize_qcbm_parameters(self, prior_scores, epoch=None):
        ...
        def objective_function(params_vector):
            ...
            target_np_safe = pnp.clip(target_np, eps, None)
            kl = pnp.sum(target_np_safe * pnp.log(target_np_safe / full_probs_safe))
        ...
        def optimization_callback(xk):
            ...
            try:
                ...
                full_probs_safe = pnp.clip(full_probs, eps, None)
                target_np_safe = pnp.clip(target_np, eps, None)
                current_kl = pnp.sum(target_np_safe * pnp.log(target_np_safe / full_probs_safe))
        ...
        full_probs_safe = pnp.clip(full_probs, eps, None)
        target_np_safe = pnp.clip(target_np, eps, None)
        kl_divergence = pnp.sum(target_np_safe * pnp.log(target_np_safe / full_probs_safe))
        
        # 只在第一个epoch时删除已存在的KL散度日志文件
        if epoch == 1 and os.path.exists('KL_divergency_dev1.4.log'):
            os.remove('KL_divergency_dev1.4.log')
        # 保存KL散度到日志文件
        with open('KL_divergency_dev1.4.log', 'a') as f:
        ...
        if epoch == 1 and os.path.exists('qcbm_optimization_results_dev1.4.log'):
            os.remove('qcbm_optimization_results_dev1.4.log')
        # 保存目标概率分布和最终QCBM概率分布到日志文件
        with open('qcbm_optimization_results_dev1.4.log', 'a') as f:
            # 保存目标概率分布（以列表格式）
        ...
        # Stats logging for target_probs and qcbm_probs
        try:
            # Convert to numpy arrays safely
            tp = np.asarray(target_np)
            qp = np.asarray(full_probs)
            # Target stats
            t_min = float(tp.min())
            t_max = float(tp.max())
            t_mean = float(tp.mean())
            t_std = float(tp.std(ddof=0))
            t_ratio = float(t_max / (t_mean + 1e-12))
            q_percentiles = np.array([0.01, 0.05, 0.10, 0.25, 0.50, 0.75, 0.90, 0.95, 0.99])
            t_qvals = np.quantile(tp, q_percentiles).tolist()
            t_qstr = '[' + ', '.join(f"{x:.6g}" for x in t_qvals) + ']'
            # QCBM stats
            q_min = float(qp.min())
            q_max = float(qp.max())
            q_mean = float(qp.mean())
            q_std = float(qp.std(ddof=0))
            q_ratio = float(q_max / (q_mean + 1e-12))
            q_qvals = np.quantile(qp, q_percentiles).tolist()
            q_qstr = '[' + ', '.join(f"{x:.6g}" for x in q_qvals) + ']'
            # Counter
            global TARGET_PROBS_Q9_LOG_COUNTER
            try:
                TARGET_PROBS_Q9_LOG_COUNTER += 1
            except Exception:
                TARGET_PROBS_Q9_LOG_COUNTER = 1
            with open('target_probs_q8_6_fuxian.log', 'a') as lf:
                if epoch is not None:
                    lf.write(f"{TARGET_PROBS_Q9_LOG_COUNTER} | Epoch {epoch} | target: min={t_min:.6g} max={t_max:.6g} mean={t_mean:.6g} std={t_std:.6g} p_max/p_mean={t_ratio:.6g} quantiles(1,5,10,25,50,75,90,95,99)={t_qstr} | qcbm: min={q_min:.6g} max={q_max:.6g} mean={q_mean:.6g} std={q_std:.6g} p_max/p_mean={q_ratio:.6g} quantiles(1,5,10,25,50,75,90,95,99)={q_qstr}\n")
                else:
                    lf.write(f"{TARGET_PROBS_Q9_LOG_COUNTER} | target: min={t_min:.6g} max={t_max:.6g} mean={t_mean:.6g} std={t_std:.6g} p_max/p_mean={t_ratio:.6g} quantiles(1,5,10,25,50,75,90,95,99)={t_qstr} | qcbm: min={q_min:.6g} max={q_max:.6g} mean={q_mean:.6g} std={q_std:.6g} p_max/p_mean={q_ratio:.6g} quantiles(1,5,10,25,50,75,90,95,99)={q_qstr}\n")
        except Exception as e:
            try:
                with open('target_probs_q8_6_fuxian.log', 'a') as lf:
                    lf.write(f"LOGGING_ERROR: {repr(e)}\n")
            except:
                pass
