...
for file in tqdm(train_file_list):
    ...
    if len(sequences) == 0:
        print(f"Warning: no FASTA records in {file}, skipping.")
        continue
...
def featurize(batch):
    ...
    for i, b in enumerate(batch):
        ...
        if seq_len != coords_len:
            print(f"Warning: length mismatch for {b['name']}: seq={seq_len}, coords={coords_len}. Using min length {eff_len}.")
    ...
    def compute_package_scores(self, X, S, mask, lengths, prior_samples, sequence_indices, pdb_ids):
        ...
        with torch.no_grad():
                ...
                try:
                    ...
                    if pdb_id is not None:
                        ...
                        if ref_matrix_path is None:
                            print(f"Warning: no reference matrix file found for {pdb_id} in {matrices_dir}. Using identity.")
                        else:
                            try:
                                ref_matrix = load_reference_matrix(ref_matrix_path)
                                if ref_matrix.shape[0] != seq_len or ref_matrix.shape[1] != seq_len:
                                    warning_msg = f"Warning: reference matrix shape mismatch for {pdb_id}: {ref_matrix.shape} vs seq_len={seq_len}. Using identity."
                                    print(warning_msg)
                                    with open('ref_matrix_debug.log', 'a') as f:
                                        f.write(warning_msg + '\n')
                                    ref_matrix = np.eye(seq_len)
                            except Exception as e:
                                print(f"Warning: failed to load reference matrix for {pdb_id} from {ref_matrix_path}: {e}. Using identity.")
                                ref_matrix = np.eye(seq_len)
                    ...
                    if base_pair_score == 0:
                        ...
                        with open('base_pair_q9_1_dev1.2.log', 'a') as f:
                             f.write(f"NON-ZERO base_pair_scor: {base_pair_score}%\n")
            ...
            if total_samples > 0:
                ...
                # 将zero_percentage写入base_pair.log文件
                #with open('base_pair.log', 'a') as f:
                 #   f.write(f"Zero base_pair_score percentage: {zero_percentage:.2f}%\n")
            print(f"{'='*50}\n")
...
if __name__ == "__main__":
    ...
    for i, split_seed in enumerate(split_seeds):
        ...
        results_df.to_csv('qcbm_rna_results_q9_1_dev1.2_qbits9.csv', index=False)
        print(f"Results updated and saved to qcbm_rna_results_q9_1_dev1.2_qbits9.csv")
    ...
    results_df.to_csv('qcbm_rna_results_q9_1_dev1.2_qbits9.csv', index=False)
    print(f"\nResults saved to qcbm_rna_results_q9_1_dev1.2_qbits9.csv")
