...
class RIdiffusion(nn.Module):
    ...
    def sample_p_zs_given_zt(self,t,s,zt,data,cond,diverse,step,last_step, prior_bits_per_graph=None, w=None):
        """
        sample zs~p(zs|zt)
        """
        ...
        # Inject QCBM prior bias into logits if provided
        if prior_bits_per_graph is not None:
            ...
            pred = pred + weight * per_node_prior
    ...
    def sample(self,data,cond = False,temperature=1.0,stop = 0, prior_bits_per_graph=None, w=None):
        ...
        for s_int in tqdm(list(reversed(range(stop, self.timesteps)))): #500
            ...
            zt , final_predicted_X  = self.sample_p_zs_given_zt(t_norm, s_norm,zt, data,cond,temperature,last_step=s_int==stop, prior_bits_per_graph=prior_bits_per_graph, w=w)
        return zt,final_predicted_X
    
    def ddim_sample(self,data,cond = False,diverse=False,stop = 0,step=100, prior_bits_per_graph=None, w=None):
        limit_dist = torch.ones(4)/4
        zt = self.sample_discrete_feature_noise(limit_dist = limit_dist,num_node = data.x.shape[0]) #[N,20] one hot 
        zt = zt.to(data.x.device)
        for s_int in tqdm(list(reversed(range(stop, self.timesteps,step)))): #500
            ...
            s_norm = s_array / self.timesteps
            t_norm = t_array /self.timesteps
            zt , final_predicted_X  = self.sample_p_zs_given_zt(t_norm, s_norm,zt, data,cond,diverse,step,last_step=s_int==stop, prior_bits_per_graph=prior_bits_per_graph, w=w)
        return zt,final_predicted_X
