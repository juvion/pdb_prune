...
For each position (i,j) in the upper triangle:
  ...
  IF REFERENCE HAS NO PAIR at (i,j):
    - Correctly predicted unpaired (no valid base pair): 1 point
    - Incorrectly predicted a pair: 0 points
...
class RNASequenceScorer:
    ...
    def __init__(self, use_numba: bool = True, min_separation: int = 4):
        """
    ...
    def calculate_base_pair_score(self, predicted_seq: str, reference_seq: str, 
        ...
        # Convert to indices for fast lookup
        pred_indices = np.array([self.base_to_idx.get(b, -1) for b in predicted_seq], dtype=np.int8)
        ref_indices = np.array([self.base_to_idx.get(b, -1) for b in reference_seq], dtype=np.int8)
        
        # Ensure matrix is contiguous and integer-typed for Numba
        reference_matrix = np.ascontiguousarray(reference_matrix.astype(np.int8))

        # Use JIT-compiled version if available, otherwise pure Python
        if self.use_numba:
            try:
                ...
                # fall back to pure Python implementation to ensure scoring completes.
                print(f"Warning: Numba JIT failed ({e}). Falling back to Python scoring.")
                total_score, paired_positions, unpaired_positions, paired_correct, unpaired_correct = \
                    self._score_loop_python(predicted_seq, reference_seq, reference_matrix)
        else:
            total_score, paired_positions, unpaired_positions, paired_correct, unpaired_correct = \
                self._score_loop_python(predicted_seq, reference_seq, reference_matrix)
        
        # Calculate maximum possible score
        max_possible_score = 4 * paired_positions + 1 * unpaired_positions
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if ref_has_pair:
                    ...
                    if pred_pair not in self.valid_pairs:
                        total_score += 1
                        unpaired_correct += 1
    ...
    def calculate_recovery_rate_score(self, predicted_seq: str, reference_seq: str) -> Tuple[float, Dict]:
        ...
        n = len(predicted_seq)
        correct_positions = sum(1 for i in range(n) if predicted_seq[i] == reference_seq[i])
    ...
    def print_detailed_results(self, results: Dict, predicted_seq: str, reference_seq: str):
        ...
        print(f"  {'Raw score:':<30} {bp_stats['raw_score']}/{bp_stats['max_possible_score']}")
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if ref_has_pair:
                ...
                # Check if prediction is NOT a valid pair (inverse of above)
                if not (
                    (pred_i == 0 and pred_j == 1) or
                    (pred_i == 1 and pred_j == 0) or
                    (pred_i == 2 and pred_j == 3) or
...
Examples:
  ...
  python rna_scorer.py predicted.fasta reference.fasta matrix.csv --min-sep 5
    ...
    # Initialize scorer
    scorer = RNASequenceScorer(use_numba=not args.no_numba, min_separation=args.min_sep)
