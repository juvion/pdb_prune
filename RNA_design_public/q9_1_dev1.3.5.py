...
class SimpleQCBM:
    ...
    def __init__(self, num_qubits, num_layers, total_sequences, device='cpu'):
        ...
        self.weights = pnp.random.randn(num_layers, num_qubits, 3) * 0.1
...
class QCBMTrainer:
    ...
    def _optimize_qcbm_parameters(self, prior_scores, epoch=None):
        ...
        def objective_function(params_vector):
            ...
            # 提取目标唯一样本对应的概率，按unique_sample_keys的顺序
            pred_probs = []
            for sample_key in self.unique_sample_keys:
                # 将样本比特串转为十进制索引（与qml.probs的顺序一致）
                idx = int(''.join([str(int(bit)) for bit in sample_key]), 2)
                pred_probs.append(full_probs[idx])

            pred_probs = pnp.array(pred_probs, dtype=float)

            # 将预测概率限制在相同支持集并归一化为分布
            eps = 1e-12
            pred_probs = pnp.clip(pred_probs, eps, None)
            sum_probs = pnp.sum(pred_probs)
            if sum_probs <= eps:
                pred_probs = pnp.ones_like(pred_probs) / len(pred_probs)
            else:
        ...
        # 计算最终的概率分布和KL散度（使用解析概率）
        full_probs = self.qcbm_model.qcircuit_probs(self.qcbm_model.weights)
        final_pred_probs = []
        for sample_key in self.unique_sample_keys:
            idx = int(''.join([str(int(bit)) for bit in sample_key]), 2)
            final_pred_probs.append(full_probs[idx])
        
        final_pred_probs = pnp.array(final_pred_probs, dtype=float)
        # 在相同支持集上归一化
        sum_probs = pnp.sum(final_pred_probs)
        if sum_probs > 0:
            final_pred_probs = final_pred_probs / sum_probs
        else:
            final_pred_probs = pnp.ones(len(self.unique_sample_keys)) / len(self.unique_sample_keys)
        ...
        eps = 1e-12
        final_pred_probs_safe = pnp.clip(final_pred_probs, eps, None)
        kl_divergence = pnp.sum(target_np * pnp.log(target_np / final_pred_probs_safe))
        ...
        with open('qcbm_optimization_results_dev1.4.log', 'a') as f:
            ...
            # 保存QCBM概率分布
            pred_str = ' '.join([f"{prob:.8f}" for prob in final_pred_probs])
            if epoch is not None:
