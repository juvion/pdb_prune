            ...
            for i, length in enumerate(lengths):
                ...
                try:
                    ...
                    combined_score_result = scorer.calculate_combined_score(
                        pred_seq_loaded, ref_seq_loaded, ref_matrix, lambda_param=0.5,seq_score_type='recovery_rate'
                    )

                    # 统计base_pair_score是否为0
                    base_pair_score = combined_score_result['base_pair_score']
                    sequence_score = combined_score_result.get('sequence_score', combined_score_result.get('edit_distance_score'))
                    seq_score_type = combined_score_result.get('sequence_score_type', 'recovery_rate')
                    total_samples += 1
                    ...
                    if base_pair_score == 0:
                        ...
                        with open('base_pair_q9_1_dev1.5.log', 'a') as f:
                            f.write(f"{seq_score_type}_score: {sequence_score}\n")
...
class QCBMTrainer:
    ...
    def _compute_prior_scores(self, train_loader):
        ...
        with open('all_scores_dev1.5.log', 'a') as f:
            list_str = '[' + ', '.join([str(float(score)) for score in unique_scores]) + ']'
            f.write('all_scores ' + list_str + '\n')
        target_probs = F.softmax(unique_scores, dim=0)
        with open('all_scores_dev1.5.log', 'a') as f:
            tp_iter = target_probs.detach().cpu().tolist() if hasattr(target_probs, 'detach') else (target_probs.tolist() if hasattr(target_probs, 'tolist') else list(target_probs))
        ...
        print(f"Collected scores: {len(unique_scores)}")
