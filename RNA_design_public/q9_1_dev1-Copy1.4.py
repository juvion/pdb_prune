...
class SimpleQCBM:
    ...
    def __init__(self, num_qubits, num_layers, total_sequences, device='cpu'):
        ...
        # 使用 StronglyEntanglingLayers 的统一权重参数 (num_layers, num_qubits, 3)

        self.weights = pnp.random.uniform(0.0, 2 * pnp.pi, size=(num_layers, num_qubits, 3))
        ...
        # 创建量子电路用于概率计算（返回所有基态的概率向量）
        self.qcircuit_probs = qml.QNode(self._circuit_probs, self.qml_device_probs, interface='numpy')

    def _circuit_sample(self, params):
        """定义参数化量子电路用于采样"""
        # 使用 PennyLane 的 StronglyEntanglingLayers 模板实现参数化电路
    ...
    def set_parameters(self, params_vector):
        ...
        # 重新整形参数
        self.weights = params_vector.reshape(self.num_layers, self.num_qubits, 3)
    ...
    def load_state_dict(self, state_dict):
        ...
        if 'weights' in state_dict:
            ...
            self.weights = state_dict['rotation_params']
        else:
            self.weights = pnp.random.randn(self.num_layers, self.num_qubits, 3) * 0.1
            ...
            for i, length in enumerate(lengths):
                ...
                try:
                    ...
                    combined_score_result = scorer.calculate_combined_score(
                        pred_seq_loaded, ref_seq_loaded, ref_matrix, lambda_param=0.9
                    )
...
class QCBMTrainer:
    ...
    def _optimize_qcbm_parameters(self, prior_scores, epoch=None):
        ...
        def objective_function(params_vector):
            ...
            # 计算交叉熵损失 H(target, pred): 直接使用full_probs与target_np
            eps = 1e-12
            cross_entropy_loss = -pnp.sum(target_np * pnp.log(pnp.clip(full_probs, eps, None)))
        ...
        with open('qcbm_optimization_results_dev1.4.log', 'a') as f:
            # 保存目标概率分布
            target_str = ' '.join([f"{prob:.8f}" for prob in target_np])
            if epoch is not None:
                f.write(f"Epoch {epoch} Target: {target_str}\n")
            else:
                f.write(f"Target: {target_str}\n")
            # 保存QCBM概率分布
            pred_str = ' '.join([f"{prob:.8f}" for prob in full_probs])
            if epoch is not None:
