...
class TrainConfig:
    ...
    lr: float = 0.001
    output_dir: str = 'ckpts/q9_1_dev1.3'
    ckpt_path: str = 'ckpts/q9_1_dev1.3/best_q9_1_dev1.3.pt'
    best_qcbm_path: str = 'ckpts/q9_1_dev1.3/best_qcbm.pt'  # 保存最佳QCBM参数
            ...
            for i, length in enumerate(lengths):
                ...
                try:
                    ...
                    if base_pair_score == 0:
                        ...
                        if epoch == 1 and os.path.exists('base_pair_q9_1_dev1.3.log'):
                            os.remove('base_pair_q9_1_dev1.3.log')
                        with open('base_pair_q9_1_dev1.3.log', 'a') as f:
...
class QCBMTrainer:
    ...
    def train_qcbm_epoch(self, train_loader, optimizer, epoch=None):
        ...
        prior_scores = self._compute_prior_scores(train_loader, epoch)
    ...
    def _compute_prior_scores(self, train_loader, epoch=None):
        ...
        with torch.no_grad():
            for batch in tqdm(train_loader, desc="Computing scores"):
                ...
                package_scores = self.rna_model_with_qcbm.compute_package_scores(
                    X, S, mask, lengths, self.prior_samples, sequence_indices, batch_pdb_ids, epoch
                )
    ...
    def _optimize_qcbm_parameters(self, prior_scores, epoch=None):
        ...
        def objective_function(params_vector):
            ...
            counts = self.qcbm_model.get_sample_counts(num_samples=20000)
        ...
        for sample_key in self.unique_sample_keys:
            sample_str = ''.join([str(int(bit)) for bit in sample_key])
            count = counts.get(sample_str, 0)
            final_pred_probs.append(count)
        
        final_pred_probs = pnp.array(final_pred_probs, dtype=float)
        total_samples = pnp.sum(final_pred_probs)
        if total_samples > 0:
            final_pred_probs = final_pred_probs / total_samples
        else:
        ...
        eps = 1e-8
        final_pred_probs_safe = final_pred_probs + eps
        kl_divergence = pnp.sum(target_np * pnp.log(target_np / final_pred_probs_safe))
        
        # 只在第一个epoch时删除已存在的KL散度日志文件
        if epoch == 1 and os.path.exists('KL_divergency.log'):
            os.remove('KL_divergency.log')
        # 保存KL散度到日志文件
        with open('KL_divergency.log', 'a') as f:
        ...
        if epoch == 1 and os.path.exists('qcbm_optimization_results.log'):
            os.remove('qcbm_optimization_results.log')
        # 保存目标概率分布和最终QCBM概率分布到日志文件
        with open('qcbm_optimization_results.log', 'a') as f:
            # 保存目标概率分布
...
if __name__ == "__main__":
    ...
    for i, split_seed in enumerate(split_seeds):
        ...
        results_df.to_csv('qcbm_rna_results_q9_1_dev1.3_qbits9.csv', index=False)
        print(f"Results updated and saved to qcbm_rna_results_q9_1_dev1.3_qbits9.csv")
    ...
    results_df.to_csv('qcbm_rna_results_q9_1_dev1.3_qbits9.csv', index=False)
    print(f"\nResults saved to qcbm_rna_results_q9_1_dev1.3_qbits9.csv")
