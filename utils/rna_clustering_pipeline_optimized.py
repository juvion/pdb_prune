#!/usr/bin/env python3
"""
Optimized RNA Clustering and Data Splitting Pipeline

An optimized version that handles large datasets efficiently by:
1. Using sampling for structure clustering to avoid O(n²) complexity
2. Adding progress tracking and resumption capabilities
3. Providing options to skip computationally expensive steps

Usage:
    python rna_clustering_pipeline_optimized.py --pdb_dir <path> --fasta_dir <path> --output_dir <path>
"""
...
class OptimizedRNAClusteringPipeline:
    def __init__(self, pdb_dir, fasta_dir, output_dir, max_seq_len=1000, 
                 max_struct_samples=1000, skip_structure=False):
        self.pdb_dir = Path(pdb_dir)
        self.fasta_dir = Path(fasta_dir)
        self.output_dir = Path(output_dir)
        self.max_seq_len = max_seq_len
        self.max_struct_samples = max_struct_samples  # Limit for structure clustering
        ...
        self.tm_scores_cache = self.output_dir / "tm_scores_cache.pkl"
    ...
    def run_sequence_clustering(self, multifasta_file):
        ...
        for threshold in self.seq_thresholds:
            ...
            # Run CD-HIT (cd-hit-est for nucleotide sequences)
            cmd = [
    ...
    def sample_for_structure_clustering(self, samples):
        ...
        for sample in samples:
            length_bin = sample['length'] // 50  # Group by 50nt bins
    ...
    def calculate_tm_scores(self, samples):
        """Calculate TM-scores between sampled pairs using US-align."""
        ...
        tm_scores = {}
        total_pairs = len(struct_samples) * (len(struct_samples) - 1) // 2
        
        logger.info(f"Computing {total_pairs} pairwise TM-scores...")
        
        with tqdm(total=total_pairs, desc="TM-score calculation") as pbar:
                ...
                for j, sample2 in enumerate(struct_samples[i+1:], i+1):
                    ...
                    try:
                        ...
                        if hasattr(e, 'returncode') and e.returncode < 0:
                            ...
                            logger.debug(f"US-align crashed for {pdb1}-{pdb2}: Signal {abs(e.returncode)}")
                        else:
                            logger.debug(f"US-align failed for {pdb1}-{pdb2}: {e}")
                        tm_scores[(pdb1, pdb2)] = 0.0
                        tm_scores[(pdb2, pdb1)] = 0.0
                    except Exception as e:
                        # Catch any other unexpected errors
                        logger.debug(f"Unexpected error for {pdb1}-{pdb2}: {e}")
                        tm_scores[(pdb1, pdb2)] = 0.0
                        tm_scores[(pdb2, pdb1)] = 0.0
    ...
    def run_pipeline(self):
        ...
        logger.info("Starting optimized RNA clustering pipeline...")
        ...
        if not self.skip_structure:
            logger.info(f"Starting structure clustering with max {self.max_struct_samples} samples...")
...
def main():
    parser = argparse.ArgumentParser(description="Optimized RNA Clustering and Data Splitting Pipeline")
    ...
    parser.add_argument("--max_struct_samples", type=int, default=1000, 
