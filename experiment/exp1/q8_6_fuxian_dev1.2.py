...
# dev_1.2 修改
# - 修正了碱基映射 base_map，统一为 featurize 所用的 'AUCG'：0->A, 1->U, 2->C, 3->G。
# - 在生成 ref_matrix（当前为单位矩阵）后，将其写入一个临时 .mtx 文件，并在评分结束后清理该临时文件。
...
# 对文件列表进行排序以确保可复现性
seqs_dir = "./RNAdesignv1/train/seqs"
train_file_list = sorted([
    f for f in os.listdir(seqs_dir)
    if not f.startswith('.') and f.lower().endswith(('.fasta', '.fa', '.fna', '.txt'))
])
content_dict = {
    "pdb_id": [],
    "seq": []
...
for file in tqdm(train_file_list):
    sequences = read_fasta_biopython(os.path.join(seqs_dir, file))
    if len(sequences) == 0:
...
def featurize(batch):
    alphabet = 'AUCG'
    B = len(batch)
    # Robust lengths: use the minimum of sequence length and coords length per item
    lengths_list = []
    for b in batch:
        seq_len = len(b['seq'])
        coords_len = len(b['coords']["P"])  # use P atom length as representative
        if seq_len != coords_len:
            print(f"Warning: length mismatch for {b['name']}: seq_len={seq_len}, coords_len={coords_len}. Using min={min(seq_len, coords_len)}")
        lengths_list.append(min(seq_len, coords_len))
    ...
    for i, b in enumerate(batch):
        # stack coords and trim to matched length
        x = np.stack([np.nan_to_num(b['coords'][c], nan=0.0) for c in ["P", "O5'", "C5'", "C4'", "C3'", "O3'"]], 1)
        l_i = int(lengths[i])
        x = x[:l_i, ...]  # trim if coords_len > seq_len
        # pad to L_max
        x_pad = np.pad(x, [[0, L_max-l_i], [0,0], [0,0]], 'constant', constant_values=(np.nan, ))
        X[i,:,:,:] = x_pad
        # sequence indices trimmed to l_i
        indices = np.asarray([alphabet.index(a) for a in b['seq'][:l_i]], dtype=np.int32)
        S[i, :l_i] = indices
        names.append(b['name'])
...
class RNAModelWithQCBM(nn.Module):
    ...
    def forward(self, X, S, mask, prior_samples=None, sequence_indices=None):
        ...
        if prior_samples is not None and sequence_indices is not None:
            ...
            # 将先验嵌入添加到对应的节点嵌入
            h_V = h_V + prior_embeddings  # 逐元素相加
    ...
    def compute_package_scores(self, X, S, mask, lengths, prior_samples, sequence_indices, pdb_ids):
        ...
        with torch.no_grad():
            ...
            # 碱基映射：0->A, 1->U, 2->C, 3->G  (match featurize's 'AUCG')
            ...
            for i, length in enumerate(lengths):
                ...
                try:
                    ...
                    # 加载参考碱基对矩阵（来自 RNAdesignv1/train/matrices）
                    matrices_dir = './RNAdesignv1/train/matrices'
                    pdb_id = pdb_ids[i]
                    ref_matrix_path = os.path.join(matrices_dir, f"{pdb_id}.txt")
                    
                    # 如果文件不存在，尝试备用命名或回退到单位矩阵
                    if not os.path.exists(ref_matrix_path):
                        # 备用：有些文件可能大小写或链标记不同，尝试常见变体
                        candidates = [
                        ...
                        if found is not None:
                            ref_matrix_path = found
                        else:
                            # 回退：单位矩阵（长度匹配）
                            ref_matrix = np.eye(len(true_sequence), dtype=int)
                    
                    # 如果还未创建 ref_matrix，则从文件读取
                    if 'ref_matrix' not in locals():
                        ref_matrix = load_reference_matrix(ref_matrix_path)
                    
                    # 验证矩阵尺寸与序列长度匹配
                    if ref_matrix.shape[0] != len(true_sequence) or ref_matrix.shape[1] != len(true_sequence):
                        # 尝试截断或填充到匹配长度（保守策略：如不匹配则回退为单位矩阵）
                        ref_matrix = np.eye(len(true_sequence), dtype=int)
                    ...
                    print(f"Error computing package score for sequence {i} (pdb_id={pdb_ids[i]}): {e}")
                    # 如果出错，使用简单的序列相似度作为备选
                    similarity = (pred_seq == true_seq).sum().float() / length
                    package_scores.append(similarity.cpu().numpy())
                
                # 确保不影响后续序列
                if 'ref_matrix' in locals():
                    del ref_matrix
                start_idx = end_idx
...
if __name__ == "__main__":
    print("Starting Q8 experiments with prior sample dataset and sequence-level recovery rate...")
    num_experiments = 5
    ...
    results_df = pd.DataFrame({
        ...
        'best_val_recovery': val_recoveries,
        'test_recovery': test_recoveries
    })

    results_df.to_csv('qcbm_rna_results_q8.csv', index=False)
    print(f"\nResults saved to qcbm_rna_results_q8.csv")
