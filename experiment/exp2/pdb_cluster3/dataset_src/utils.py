...
def dataset_argument(n=0):
    ...
    if n == 0:
        ...
        dataset_arg['c_4_max_neighbors'] = 10
    elif n == 1:
        ...
        dataset_arg['divide_num'] = 1
        dataset_arg['divide_idx'] = 0
        dataset_arg['c_4_max_neighbors'] = 20
    elif n == 9:
        ...
        dataset_arg['divide_num'] = 1
        dataset_arg['divide_idx'] = 0
        dataset_arg['c_4_max_neighbors'] = 5
    elif n == 18:
    ...
    #     dataset_arg['divide_num'] = 1
    #     dataset_arg['divide_idx'] = 0
    #     dataset_arg['c_4_max_neighbors'] = 15
    #     dataset_arg['struc_2nds_res_path'] = 'dataset/SS_new'
    #     dataset_arg['diffusion'] = False
    #     dataset_arg['pre_equivariant']  = False
    elif n == 666:
        ...
        dataset_arg['divide_num'] = 1
        dataset_arg['divide_idx'] = 0
        dataset_arg['c_4prime_max_neighbors'] = 15
        dataset_arg['struc_2nds_res_path'] = 'dataset/SS_new'
        dataset_arg['diffusion'] = False
        dataset_arg['pre_equivariant']  = False
    return dataset_arg
...
def get_stat(graph_root, limited_num=None, num_subgroup=1000, max_limits=100000):
    ...
    saved_filename = os.path.join(
        '/'.join(graph_root.split('/')[:-1]), filename + '_rnas.txt')
    with open(saved_filename, 'w') as f:
        for i in range(n):
            f.write(str(filenames[i]) + '\n')

    saved_filename = os.path.join(
        '/'.join(graph_root.split('/')[:-1]), filename + '_wrong_rnas.txt')
    with open(saved_filename, 'w') as f:
        for file in wrong_rnas:
            f.write(file + '\n')
...
class NormalizeRNA(BaseTransform):
    r"""Centers and normalizes node positions to the interval :math:`(-1, 1)`
    (functional name: :obj:`normalize_scale`).
    """

    def __init__(self, filename, skip_x=4, skip_edge_attr=64, safe_domi=1e-10):
...
def seq_dist_distrib(loader):
    ...
    plt.ylabel('Times')
    plt.savefig('rna/dataset_alpha_Fold/Val_seq_dist' + str(bins) + '.png')
    return seq_dist
