from dataset_src.utils import normalize, safe_index, one_hot_res, log, dihedral, NormalizeRNA, dataset_argument, get_stat
import argparse
...
class RNA_imem(InMemoryDataset):
    r"""
    Args:
        ...
        c_4prime_max_neighbors (int, optional): The number of maximum 
    ...
    allowable_features = {
        ...
        # 'possible_nucleotides': ['ALA', 'ARG', 'ASN', 'ASP', 'CYS', 'GLN', 'GLU', 'GLY', 'HIS', 'ILE', 'LEU', 'LYS', 'MET',
    ...
    def __init__(self, root: str, name: str,
                 split: str = 'train',
                 num_residue_type: int = 4,
                 micro_radius: int = 20,
                 c_4prime_max_neighbors: int = 10,
                 cutoff: int = 30,
                 seq_dist_cut: int = 64,
        ...
        self.c_4prime_max_neighbors = c_4prime_max_neighbors
        ...
        self.backbone_list=['P','O5\'','C5\'','C4\'','C3\'','O3\'']
    ...
    def get_EMD(self,input1,input2):
        all_emd = []
        # print(input1)
        ...
        for i in input1:
            ...
            for j in input2:
                # print(jKK)
                emd = doRubnerComparison(i,j)
                dis_map.append(emd)

            all_emd.append(dis_map)
        # all_emd = map(doRubnerComparison,input1)
        # print(all_emd)


        return np.array(all_emd)
        ...
        for k in range(3):
            ...
            for file in filenames_list[k]:
                ...
                if self.is_normalize:
                    normalize_transform = NormalizeRNA(filename=self.normalize_file)
...
#             if name in ['1bteA00', '1byiA00','1c1yB00']:
            ...
            rec, rec_coords, c_4prime_coords, p_coords, n_coords, backbond_coords= self.get_receptor_inference(protein_filename)
            ...
            if struc_2nd_res is not None:
                if c_4prime_coords.shape[0] == struc_2nd_res.shape[0]:
                    rec_graph = self.get_c4prime_graph(
                        rec, c_4prime_coords, p_coords, n_coords, rec_coords, struc_2nd_res,backbond_coords)
                    if not rec_graph:
                        continue
                    # print(rec_graph)
    ...
    def rec_residue_featurizer(self, rec, one_hot=True, add_feature=None):
        ...
        for add_feature_1 in add_feature:
            ...
            #print("add_feature_1add_feature_1",add_feature_1)
            num_feature += add_feature_1.shape[1]
            #print(num_feature)
        res_feature = torch.zeros(num_res, self.num_residue_type + num_feature)
        #print("222222",num_res)
        #print("333333",num_feature)
        #print("444444",self.num_residue_type)
        count = 0
        # print('res feature:')
        ...
        try:
            ...
            for residue in rec.get_residues():
                ...
                residx = safe_index(
                    self.allowable_features['possible_nucleotides'], residue.get_resname())
                # print(residx)
    ...
    def get_node_features(self, p_coords, n_coords, c_4prime_coords, coord_mask, with_coord_mask=True, use_angle=False, use_omega=False):
        num_res = p_coords.shape[0]
        if use_omega:
            num_angle_type = 3
            ...
            for i in range(num_res-1):
                ...
                angles[i, 0] = dihedral(
                    n_coords[i], p_coords[i], c_4prime_coords[i], p_coords[i+1])

                # psi involves the backbone atoms N-Cα-C-N.
                angles[i, 1] = dihedral(
                    p_coords[i], c_4prime_coords[i], n_coords[i], p_coords[i+1])
                angles[i, 2] = dihedral(
                    c_4prime_coords[i], n_coords[i], p_coords[i+1], c_4prime_coords[i+1])
        else:
            num_angle_type = 2
            angles = np.zeros((num_res, num_angle_type))
            for i in range(num_res-2):
                # RNA phi 2 angle 
                angles[i, 0] = dihedral(
                    c_4prime_coords[i], p_coords[i+1], c_4prime_coords[i+1], p_coords[i+2])
            for i in range(num_res-1):
                # RNA phi 4 angle
                angles[i, 1] = dihedral(
                    p_coords[i], c_4prime_coords[i], p_coords[i+1], c_4prime_coords[i+1])
        if use_angle:
    ...
    def get_c4prime_graph(self, rec, c_4prime_coords, p_coords, n_coords,rec_coords,struc_2nd_res = None):

        scalar_feature, vec_feature = self.get_node_features(
            p_coords, n_coords, c_4prime_coords, coord_mask=None, with_coord_mask=False, use_angle=self.use_angle, use_omega=self.use_omega)
        ...
        for i, residue in enumerate(rec.get_residues()):
            p_coord = p_coords[i]
            c_4prime_coord = c_4prime_coords[i]
            n_coord = n_coords[i]
            u_i = (p_coord - c_4prime_coord) / \
                np.linalg.norm(p_coord - c_4prime_coord)
            t_i = (n_coord - c_4prime_coord) / \
                np.linalg.norm(n_coord - c_4prime_coord)
            n_i = np.cross(u_i, t_i) / \
                np.linalg.norm(np.cross(u_i, t_i))   # main chain
            v_i = np.cross(n_i, u_i)
            assert (math.fabs(
                np.linalg.norm(v_i) - 1.) < 1e-5), "v_i norm larger than 1"
            n_i_list.append(n_i)
            u_i_list.append(u_i)
            v_i_list.append(v_i)
            residue_representatives_loc_list.append(c_4prime_coord)
        ...
        num_residues = len(c_4prime_coords)
        if num_residues <= 1:
        ...
        ###################################
        #######if use edit distance #######
        ###################################
        if_edit_distance = False
        # print(backbone_atm_coords)
        if if_edit_distance:
            # bb_atoms = np.array(backbone_atm_coords).T
            backbone_atm_coords=backbone_atm_coords[0]

            distances = self.get_EMD(backbone_atm_coords,backbone_atm_coords)
            print(distances.shape)
        else:
            distances = spa.distance.cdist(c_4prime_coords, c_4prime_coords)
            # print(distances.shape)
        ...
        for i in range(num_residues):
            ...
            dst.remove(i)
            if self.c_4prime_max_neighbors != None and len(dst) > self.c_4prime_max_neighbors:
                dst = list(np.argsort(distances[i, :]))[
                    1: self.c_4prime_max_neighbors + 1]
            if len(dst) == 0:
        ...
        # print(len(src_list),len(dst_list),len(dist_list))
        ...
        if x is not None:
            ...
            # self.get_c4prime_graph_single(graph, 6)
            return graph
        else:
            return None
    ...
    def get_receptor_inference(self, rec_path):
        ...
        for i, chain in enumerate(rec):
            chaip_coords = []  # num_residues, num_atoms, 3
            chain_c_4prime_coords = []
            chain_p_coords = []
            chain_c_coords = []
            count = 0
            invalid_res_ids = []
            ...
            for res_idx, residue in enumerate(chain):
                ...
                residue_coords = []
                bb_coords = []
                c_alpha, n, c, bb = None, None, None, None
                ...
                for atom in residue:
                    ...
                    if residue.resname in ['A', 'G'] and atom.name == 'N9':
                        c = list(atom.get_vector())
                    elif residue.resname in ['C', 'U'] and atom.name == 'N1':
                        c = list(atom.get_vector())

                    residue_coords.append(list(atom.get_vector()))
                    if atom.name in self.backbone_list:
                        bb_coords.append(list(atom.get_vector()))

                if c_alpha is not None and n is not None and c is not None:
                    chain_c_4prime_coords.append(c_alpha)
                    chain_p_coords.append(n)
                    chain_c_coords.append(c)
                    chaip_coords.append(np.array(residue_coords))
                    chain_backbone_atm.append(np.array(bb_coords).T)
                    count += 1
                else:
                    invalid_res_ids.append(residue.get_id())
            ...
            coords.append(chaip_coords)
            c_4prime_coords.append(np.array(chain_c_4prime_coords))
            p_coords.append(np.array(chain_p_coords))
            n_coords.append(np.array(chain_c_coords))
            backbone_atm_ls.append(chain_backbone_atm)
            if len(chaip_coords) > 0:
                valid_chain_ids.append(chain.get_id())

        valid_coords = []
        valid_c_4prime_coords = []
        valid_p_coords = []
        valid_n_coords = []
        valid_lengths = []
        invalid_chain_ids = []
        ...
        for i, chain in enumerate(rec):
            if chain.get_id() in valid_chain_ids:
                ...
                valid_c_4prime_coords.append(c_4prime_coords[i])
                valid_p_coords.append(p_coords[i])
                valid_n_coords.append(n_coords[i])
                valid_lengths.append(lengths[i])
                valid_backbone.append(chain_backbone_atm[i])
            else:
                invalid_chain_ids.append(chain.get_id())

        if not valid_c_4prime_coords:
            return None

        coords = [item for sublist in valid_coords for item in sublist]
        backbone = [item for sublist in valid_backbone for item in sublist]
        c_4prime_coords = np.concatenate(valid_c_4prime_coords, axis=0)
        p_coords = np.concatenate(valid_p_coords, axis=0)
        n_coords = np.concatenate(valid_n_coords, axis=0)

        for invalid_id in invalid_chain_ids:
            rec.detach_child(invalid_id)

        return rec, coords, c_4prime_coords, p_coords, n_coords
    ...
    def get(self, idx):
        ...
        if self.diffusion:
            ...
            if self.use_micro:
                generate_graph = self.get_c4prime_graph_single(
    ...
    def get_c4prime_graph_single(self, graph, idx_protein, amino_idx):
...
if __name__ == '__main__':
    ...
    train_dataset = RNA_imem(dataset_arg['root'], dataset_arg['name'], split='test',
