...
5.  **Extract and Save Continuous Loop Fragments:** Identify consecutive runs of residues that were marked as "loop-like" in step 4. These continuous segments are the loop fragments.
    ...
    * For each identified fragment containing one or more residues:
        ...
        * Construct the output filename using the original PDB code (e.g., from the input filename like `1abc.pdb` -> `1abc`), the start position, and the end position: `[pdb_code]_[start]-[end].pdb`. If the fragment ends at the last residue of the pdb, name the file with "T" at the end.
